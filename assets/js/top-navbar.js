(() => {
    ...
    const renderTopNavbar = () => {
                            ...
                            <li class="nav-item dropdown">
                                <div class="nav-dropdown">
                                    <a href="#" id="navUserMenu" class="nav-item nav-link dropdown-toggle text-secondary pe-0" data-bs-toggle="dropdown" aria-expanded="false">
                                        <i class="fas fa-user"></i> <span>${accountId}</span> <i style="font-size: .8em;" class="fas fa-caret-down"></i>
                                    </a>
        ...
        if (sidebarToggleButton) {
            let lastTouchAt = 0;
            const triggerSidebarToggle = (event) => {
                if (event) {
                    event.preventDefault();
                    event.stopPropagation();
                }
                if (typeof window.toggleSidebarLayout === "function") {
            ...
            sidebarToggleButton.addEventListener("touchstart", (event) => {
                lastTouchAt = Date.now();
                triggerSidebarToggle(event);
            }, { passive: false });
