...
if ($existingId) {
    $stmt = $db->prepare("UPDATE tenant_settings SET rental_mode = :rental_mode, updated_at = :updated_at WHERE id = :id");
    $stmt->bindValue(':id', intval($existingId));
    $stmt->bindValue(':rental_mode', $mode);
    $stmt->bindValue(':updated_at', $now);
    $stmt->execute();
} else {
    $stmt = $db->prepare("INSERT INTO tenant_settings (tenant_id, rental_mode, updated_at) VALUES (:tenant_id, :rental_mode, :updated_at)");
